...
def get_patch(input_array,i,j,filter_width,filter_height,stride):
	# will find 2D/3D itself?
	start_i = i * stride # x_location
	start_j = j * stride
...
def element_wise_option(array, op):
...
class Filter(object):
	def __init__(self,width,height,depth):
		self.weights = np.random.uniform(-1e-4,1e-4,(depth,height,widht))
		self.bias = 0
...
class ConvLayer(object):
    ...
    def forward(self,input_array):
    	...
    	self.padding_input_array = padding(input_array,
    		self.zero_padding)
    	for f in range(self.filter_number):
    		filter = self.filters[f]
    		conv(self.padding_input_array,
    			filter.get_weights,self.output_array[f],
    			self.stride , filter.get_bias)
    	element_wise_option(self.output_array,self.activator.forward)

    def backword(self,input_array,sensitivity_array,activator):
    ...
    def bp_sensitivity_map(self, senstivity_array,activator):
    	...
    	for f in range(self.filter_number):
    		filter = self.filters[f]
    		flipped_weights = np.array(map(lambda i :np.rot90(i,2),
    			filter_weights()))
    	...
    	element_wise_option(derivative_array,activator.backword)
    	self.delta_array *= derivative_array

    def bp_gradient(self,sensitivity_array):
    	expanded_array = self.expand_sensitivity_map(sensitivity_array)
    	for i in range(self.filter_number):
    		filter = self.filter_number
    		for d in range(filter.weigths.shape[0]):
    			conv(self.padding_input_array[d],
    				expanded_array[f],
