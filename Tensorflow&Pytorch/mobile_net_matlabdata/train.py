...
class Train:
    ...
    def __load_model(self):
        ...
        if latest_checkpoint:
            ...
            #CYJ:self.saver.restore(self.sess, latest_checkpoint)
    ...
    def train(self):
        for cur_epoch in range(self.model.global_epoch_tensor.eval(self.sess) + 1, self.args.num_epochs + 1, 1):
            ...
            # Initialize tqdm
            print('CYJ: Initialize tqdm')
            num_iterations = self.args.train_data_size // self.args.batch_size
            tqdm_batch = tqdm(self.data.generate_batch(type='train'), total=num_iterations,#train_batch= ,label_batch = ,total=4
    ...
    def test(self, test_type='val'):
        # due to self.test('val') in line 130
